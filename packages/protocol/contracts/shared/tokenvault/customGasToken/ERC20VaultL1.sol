...
import "src/shared/tokenvault/BaseVault.sol";

/// @title ERC20VaultL1
/// @notice This vault holds all ERC20 tokens (excluding gas token) and also ether that users have
...
/// @dev Labeled in address resolver as "erc20_vault_l1".
/// @custom:security-contact security@nethermind.io
contract ERC20VaultL1 is BaseVault {
    // Surge: This contract is used *only* when the rollup uses a custom gas token.
    // This requires a complementary ERC20VaultL2 contract on the L2 side.
    ...
    struct BridgeTransferOp {
        ...
        // not solvable.
        // Surge: Only used for L2 -> L1 bridging
        uint256 solverFee;
    }

    // Surge: address of the ERC20 token used as the native gas token on L2 side
    address public immutable l2GasToken;
    ...
    {
        {
            ...
            // Surge:
            // - The user who is bridging, pays relayer fees in gas token on L1.
            // - The relayer receives the quivalent in native gas token on L2.
            // - Logic is handled in `_handleMessage()`

            checkToAddressOnSrcChain(_op.to, _op.destChainId);
        }

        address bridge = resolve(LibStrings.B_BRIDGE_L1, false);

        (bytes memory data, CanonicalERC20 memory ctoken, uint256 balanceChangeAmount) =
            _handleMessage(_op);
        ...
        IBridge.Message memory message = IBridge.Message({
            ...
            // Surge: gas token is send as the message value
            value: _op.token == l2GasToken ? _op.amount : 0,
            fee: _op.fee,
            gasLimit: _op.gasLimit,
            // Surge: data is empty for gas token bridging since it is send as the message value
            data: _op.token == l2GasToken ? bytes("") : data
        });

        bytes32 msgHash;
        // Surge: Do not send ether with message as the vault itself now holds ether
        (msgHash, message_) = IBridge(bridge).sendMessage(message);
        ...
        emit TokenSent({
            ...
            // Surge: solver is not needed for L1 -> L2 bridging
            solverFee: 0
        });
    }
        ...
        nonReentrant
    {
        ...
        address token = _transferTokensOrEther(ctoken, _message.srcOwner, amount);
    ...
    function _handleMessage(BridgeTransferOp calldata _op)
        private
        returns (bytes memory msgData_, CanonicalERC20 memory ctoken_, uint256 balanceChangeAmount_)
    {
        if (_op.token == address(0)) {
            // Surge: Handle ether bridging
            ...
            // Surge: This will implicitly handle gas token bridging on L1 side
        ...
        if (_op.token != l2GasToken) {
            // Surge: Extract relayer fees separately if bridging non-gas token
            IERC20(_op.token).safeTransferFrom(msg.sender, address(this), _op.fee);
        }

        // Surge: Remove solver condition preparation

        // Surge: Remove solver parameters
        msgData_ = abi.encodeCall(
            this.onMessageInvocation, abi.encode(ctoken_, msg.sender, _op.to, balanceChangeAmount_)
        );
    }
