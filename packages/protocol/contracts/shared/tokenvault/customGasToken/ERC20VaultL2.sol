...
/// @title ERC20VaultL2
/// @notice This vault holds all ERC20 tokens that users have deposited. It also manages the
/// mapping between canonical ERC20 tokens and their bridged tokens. This vault does not support
/// rebase/elastic tokens.
/// @dev Labeled in address resolver as "erc20_vault_l2".
/// @custom:security-contact security@nethermind.io
contract ERC20VaultL2 is BaseVault {
    // Surge: This contract is used *only* when the rollup uses a custom gas token.
    // This requires a complementary ERC20VaultL1 contract on the L1 side.
    ...
    // Surge: address of the ERC20 token used as the bridge input for gas token on L1 side
    address public immutable l2GasToken;

    // Surge: a wrapper address for the native token on L1 side
    // This needs to be set manually in the L2 genesis.
    // This can be deterministiically calculated as:
    // keccak256(0xFF, <address_of_erc20_vault_l2>, salt, <bytecode_of_bridged_erc20>);
    ...
    constructor(
        address _resolver,
        address _l2GasToken,
        address _l1NativeTokenWrapper
    )
        BaseVault(_resolver)
    {
        l2GasToken = _l2GasToken;
        l1NativeTokenWrapper = _l1NativeTokenWrapper;
    }
    ...
    {
        // Surge: remove handling of Ether transfer
        if (_ctoken.chainId == block.chainid) {
            token_ = _ctoken.addr;
            IERC20(token_).safeTransfer(_to, _amount);
        } else {
            // Surge: When ether is used on source, this ends up deploying a bridged ether token
        ...
        )
    {
        ...
        if (_op.token == address(0)) {
            // Surge: enforce gas token to be the ctoken
            ctoken_.addr = l2GasToken;
            ctoken_.chainId = _op.destChainId;

            balanceChangeAmount_ = _op.amount;
            balanceChangeSolverFee_ = _op.solverFee;
        } else if (
            bridgedToCanonical[_op.token].addr != address(0) || _op.token == l1NativeTokenWrapper
        ) {
            // Handle bridged token
            // Surge: When dealing with Ether, this will automatically get set to ctoken with
            // address(0)
