...
/// @dev Labeled in address resolver as "bridge_l1".
/// @custom:security-contact security@nethermind.io
contract BridgeL1 is EssentialContract, IBridge {
    // Surge: This contract is used *only* when the rollup uses a custom gas token.
    // This requires a complementary BridgeL2 contract on the L2 side.
    // Besides acting as the cross chain messaging contract, this contract holds the
    // custom gas token on L1 side.
        ...
        returns (bytes32 msgHash_, Message memory message_)
    {
        ...
        if (totalValue > 0) {
            IERC20(l2GasToken).safeTransferFrom(msg.sender, address(this), totalValue);
        }
        ...
        nonReentrant
    {
        ...
        if (_message.from.supportsInterface(type(IRecallableSender).interfaceId)) {
            ...
            // needs to be handled by the initiator contract
            IRecallableSender(_message.from).onMessageRecalled(_message, msgHash);
        ...
        if (_message.fee != 0) {
            ...
            if (stats.processedByRelayer && _message.gasLimit != 0) {
                unchecked {
                    ...
                    refundAmount -= fee;
                    // Surge: Send gas token to the relayer
                    IERC20(l2GasToken).safeTransfer(msg.sender, fee);

                    // Surge: remove native eth fee transfer
                }
            }
        }

        // Surge: Initiate refund in gas token
        IERC20(l2GasToken).safeTransfer(_message.destOwner, refundAmount);

        // Surge: remove refunds in native eth
        ...
        nonReentrant
    {
        ...
        bool succeeded;
        if (_unableToInvokeMessageCall(_message, signalService)) {
            // Surge: Return gas token to the initiator
            IERC20(l2GasToken).safeTransfer(_message.destOwner, _message.value);
        ...
        returns (bool success_)
    {
        ...
        // Surge: Send gas token to the callee
        IERC20(l2GasToken).safeTransfer(to, value);

        assembly {
            // Surge: Do not send value with message as the value is send in gas token
            success_ := call(_gasLimit, to, 0, add(data, 0x20), mload(data), 0, 0)
            gasLeft := gas()
        }
    ...
    // Surge: remove ether quota check function
