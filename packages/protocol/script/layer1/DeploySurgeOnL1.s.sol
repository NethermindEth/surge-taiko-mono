...
contract DeploySurgeOnL1 is DeployCapability {
        ...
        returns (address rollupAddressManager)
    {
        ...
        deployComposedVerifiers(owner, rollupAddressManager);
    }
    ...
    function deployComposedVerifiers(address owner, address rollupAddressManager) private {
        deployProxy({
            name: "tier_tee_any",
            impl: address(new TeeAnyVerifier()),
    ...
    function deployTierRouter() private returns (address) {
        return address(new SurgeTierRouter());
    }
