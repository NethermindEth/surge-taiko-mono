...
//
// Solidity: event ProposedAndProved(bytes32 indexed proposalHash, bytes32 indexed lastFinalizedBlockHash, uint48 maxAnchorBlockNumber, uint8 basefeeSharingPctg, (bool,(bytes32[],uint24,uint48))[] sources, bytes32[] signalSlots, (uint48,bytes32,bytes32) checkpoint)
func (_RealTimeInboxClient *RealTimeInboxClientFilterer) FilterProposedAndProved(opts *bind.FilterOpts, proposalHash [][32]byte, lastFinalizedBlockHash [][32]byte) (*RealTimeInboxClientProposedAndProvedIterator, error) {
	...
	var lastFinalizedBlockHashRule []interface{}
	for _, lastFinalizedBlockHashItem := range lastFinalizedBlockHash {
		lastFinalizedBlockHashRule = append(lastFinalizedBlockHashRule, lastFinalizedBlockHashItem)
	}

	logs, sub, err := _RealTimeInboxClient.contract.FilterLogs(opts, "ProposedAndProved", proposalHashRule, lastFinalizedBlockHashRule)
	if err != nil {
		return nil, err
	}
...
//
// Solidity: event ProposedAndProved(bytes32 indexed proposalHash, bytes32 indexed lastFinalizedBlockHash, uint48 maxAnchorBlockNumber, uint8 basefeeSharingPctg, (bool,(bytes32[],uint24,uint48))[] sources, bytes32[] signalSlots, (uint48,bytes32,bytes32) checkpoint)
func (_RealTimeInboxClient *RealTimeInboxClientFilterer) WatchProposedAndProved(opts *bind.WatchOpts, sink chan<- *RealTimeInboxClientProposedAndProved, proposalHash [][32]byte, lastFinalizedBlockHash [][32]byte) (event.Subscription, error) {
	...
	logs, sub, err := _RealTimeInboxClient.contract.WatchLogs(opts, "ProposedAndProved", proposalHashRule, lastFinalizedBlockHashRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
