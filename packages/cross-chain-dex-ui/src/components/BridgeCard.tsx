...
export function BridgeCard({ onSetupWallet }: BridgeCardProps) {
  ...
  const currentBalance = isDeposit
    ? (bridgeToken === L1_NATIVE_SYMBOL ? l1EthBalance : l1UsdcBalance)
  ...
  const handleBridge = useCallback(async () => {
    ...
    if (isDeposit) {
      ...
      // L2→L1 withdrawal via builder (native only for now)
      if (!smartWallet) return;
  ...
  const getButtonText = () => {
    if (isPending) return isDeposit ? "Bridging..." : "Withdrawing...";
    ...
    if (!isDeposit && bridgeToken === "USDC") return "USDC withdrawal not yet supported";
    if (hasInsufficientBalance) return "Insufficient Balance";
    return isDeposit
      ? `Bridge ${bridgeToken} to L2`
      : `Withdraw ${bridgeToken} to L1`;
  };

  const isUsdcWithdrawal = !isDeposit && bridgeToken === "USDC";
  const isDisabled =
    isPending ||
    !isConnected ||
    !smartWallet ||
      ...
      <div className="w-full md:max-w-md bg-surge-card/80 border border-surge-border/50 rounded-2xl p-4 space-y-3 shadow-xl shadow-black/20 hover-glow transition-all duration-[1000ms] ease-[cubic-bezier(0.16,1,0.3,1)]">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => {
              ...
              setInputAmount("");
              setBridgeToken(L1_NATIVE_SYMBOL as BridgeToken);
            }}
        ...
        {isUsdcWithdrawal && (
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg px-3 py-2 text-xs text-blue-400">
            USDC withdrawals are not yet supported. Use {L1_NATIVE_SYMBOL} to withdraw.
          </div>
        )}
        ...
        <TokenInput
          ...
          label={isDeposit ? "Amount" : "Withdraw Amount"}
        />

        {/* Recipient (optional) */}
        <div className="space-y-1">
          <label className="text-xs text-gray-400">
            Recipient on {isDeposit ? "L2" : "L1"} (optional)
          </label>
          <input
            type="text"
            value={recipient}
        ...
        >
          {isPending ? (
            <span className="flex items-center justify-center gap-2">
              ...
              {isDeposit ? "Bridging..." : "Withdrawing..."}
            </span>
          ) : (
            getButtonText()
          )}
      ...
      {amountIn > 0n && (
        <div className="w-full md:max-w-sm bg-surge-card/80 border border-surge-border/50 rounded-2xl p-4 space-y-3 shadow-xl shadow-black/20 animate-panel-in">
          <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
            {isDeposit ? "Bridge Details" : "Withdrawal Details"}
          </h3>
          ...
          {!isDeposit && (
            <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg px-3 py-2 text-xs text-blue-400">
              Withdrawals are processed via the Taiko bridge. Funds will be available on L1 after the message is relayed and proven.
            </div>
          )}
        </div>
      )}
